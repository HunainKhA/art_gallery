  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        // High quality max dimension cap for super fast encoding & small PDF size
        const maxDim = 1400;
        let targetW = origW;
        let targetH = origH;
...
/**
 * Generates and downloads the luxury Exhibition Catalogue PDF:
 * Supports with/without prices, direct start on artwork 1 when no banner is uploaded,
 * borderless pages, centered captions, and high-speed parallel preloading.
 */
export const generateCatalogPDF = async (exhibition, artworks, onProgress, options = {}) => {
  ...
  // 3. Parallel preload cover, back cover, and ALL artworks
