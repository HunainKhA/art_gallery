  ...
  return (
    <div className="glass-card" style={{ padding: '2rem', maxWidth: '950px', margin: '0 auto', animation: 'fadeIn 0.4s ease' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div className="catalogue-mode-switch">
            <button
              type="button"
              className={`catalogue-mode-btn ${pdfPriceMode === 'without_price' ? 'active' : 'inactive'}`}
              onClick={() => setPdfPriceMode('without_price')}
            >
              Without Price
            </button>
            <button
              type="button"
              className={`catalogue-mode-btn ${pdfPriceMode === 'with_price' ? 'active' : 'inactive'}`}
              onClick={() => setPdfPriceMode('with_price')}
            >
              With Price (PKR)
            </button>
          </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        {/* Dedicated Price Switch Highlight Bar */}
        <div style={{
          display: 'flex',
          alignItems: 'center',
          ...
          <div>
            <div style={{ color: 'var(--text-primary)', fontSize: '0.9rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span>📄 Catalogue Price Mode:</span>
              <span style={{ color: pdfPriceMode === 'with_price' ? 'var(--accent-gold)' : 'var(--text-secondary)' }}>
                {pdfPriceMode === 'with_price' ? 'With Prices (PKR)' : 'Without Prices (Clean)'}
              </span>
            </div>
            <div style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: '0.2rem' }}>
              Choose whether to show PKR prices on every page under the artwork caption.
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            ...
            >
              {downloadingPdf ? (
                <><Loader className="animate-spin" size={14} /> {pdfProgress || 'Exporting...'}</>
              ) : (
                <><FileDown size={14} /> Download {pdfPriceMode === 'with_price' ? 'With Price' : 'Without Price'} ({formData.artwork_ids.length})</>
              )}
            </button>
          </div>
        </div>
